...
function eLinkAuxiliarDeImpressao(titulo, href) {
  const tituloNormalizado = normalizar(titulo);
  const endereco = String(href || '');
  return /^pdf\s*[-:]/i.test(tituloNormalizado)
    || /\/component\/topdf\//i.test(endereco)
    || /[?&]option=com_gmg(?:&|$)/i.test(endereco) && /[?&]controller=document(?:&|$)/i.test(endereco);
}
...
function extrairDocumentos(html, sourceUrl) {
  ...
  anchors.forEach((anchor, indice) => {
    ...
    if (!eTituloDeDocumento(titulo) || eLinkAuxiliarDeImpressao(titulo, anchor[3])) return;
...
function extrairLinksPaginacao(html, paginaAtual) {
  ...
  for (const anchor of String(html || '').matchAll(/<a\b([^>]*)href\s*=\s*(["'])(.*?)\2[^>]*>/gi)) {
    ...
    const filtrosAtuais = new URL(paginaAtual).searchParams;
    for (const [chave, valor] of filtrosAtuais) {
      if (!parsed.searchParams.has(chave)) parsed.searchParams.set(chave, valor);
    }
...
module.exports = {
  ...
  consultarMetadados,
  consultarDocumentos,
  _private: { extrairMetadados, extrairDocumentos, deduplicar, extrairLinksPaginacao, montarUrl, limparTexto, eLinkAuxiliarDeImpressao }
};
