...
// Códigos CONCLA de entidades sem fins lucrativos que podem manter serviços filantrópicos.
// A API não informa certificação CEBAS; a interface não afirma que toda entidade 3xxx é certificada.
const NATUREZAS_SEM_FINS_LUCRATIVOS = new Set(['3069', '3077', '3220', '3239', '3301', '3999']);
const cache = new Map();
...
function classificarNatureza(registro) {
  ...
  const esfera = String(registro?.descricao_esfera_administrativa || '').trim().toUpperCase();
  const publica = codigo.startsWith('1') || ['FEDERAL', 'ESTADUAL', 'MUNICIPAL', 'DISTRITAL'].includes(esfera);
  return { codigo, publica, semFinsLucrativos: NATUREZAS_SEM_FINS_LUCRATIVOS.has(codigo) };
}

function estabelecimentoElegivel(registro) {
  const natureza = classificarNatureza(registro);
  return atendeSus(registro) && (natureza.publica || natureza.semFinsLucrativos);
}
...
function normalizarEstabelecimento(registro) {
  ...
  return {
    ...
    classificacaoNatureza: natureza.publica ? 'Pública' : 'Entidade sem fins lucrativos',
    atendeSus: true,
    endereco: {
...
async function paginaDemas(ibge, offset) {
  const chave = `demas:${ibge}:${offset}`;
...
async function listarEstabelecimentos({ ibge: ibgeRecebido, cursor: cursorRecebido, pageSize: pageSizeRecebido }) {
  ...
  while (items.length < pageSize && hasNext && requisicoes < MAX_DEMAS_REQUESTS) {
    const pagina = await paginaDemas(ibge, offset);
    requisicoes += 1;
    ...
    for (const registro of pagina) {
      offset += 1;
      consumidos += 1;
      if (estabelecimentoElegivel(registro)) items.push(normalizarEstabelecimento(registro));
