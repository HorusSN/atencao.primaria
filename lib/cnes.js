...
async function listarEquipesMunicipio({ ibge: ibgeRecebido }) {
  const ibge = validarIbge(ibgeRecebido);
  const gerais = await listarEstabelecimentos({ ibge, cursor: 0, pageSize: PAGE_SIZE_MAX, incluirTodos: true });
  const equipes = await emLotes(gerais.items, 4, async unidade => {
...
async function listarProfissionaisMunicipio({ ibge: ibgeRecebido, cargo = '', cnes = '', mantenedora = '' }) {
  const ibge = validarIbge(ibgeRecebido);
  const url = new URL(`/services/profissionais`, PORTAL_ORIGIN); url.searchParams.set('municipio', ibge);
  const payload = await requisitarJson(url, { portal: true, timeoutMs: 20000 });
  if (!Array.isArray(payload)) throw erro('A fonte de profissionais retornou um formato inesperado.');
  const norm = payload.map(normalizarProfissionalMunicipal).filter(item => item.nome);
  ...
  return { ibge, items, total: items.length, source: url.toString(), consultadoEm: new Date().toISOString() };
}
