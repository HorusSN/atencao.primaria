(function () {
  ...
  function renderizarLista() {
    ...
    $('#production-indicators').innerHTML = `<article class="indicator-panel cnes-panel">
      ...
      <p class="indicator-note cnes-rule-note">Somente unidades com atendimento SUS confirmado e natureza jurídica pública ou sem fins lucrativos elegível. Empresas privadas com fins lucrativos e pessoas físicas são excluídas.</p>
  ...
  function renderizarDetalhe() {
    ...
    $('#production-indicators').innerHTML = `<article class="indicator-panel cnes-panel cnes-detail-panel">
      ...
      <header class="cnes-detail-heading"><div><p class="cnes-eyebrow">2 · Unidade</p><h3>${escapar(u.nome)}</h3><p>CNES ${escapar(u.cnes)}</p></div><span class="cnes-sus-badge">Atende ao SUS</span></header>
      <dl class="cnes-basic-data"><div><dt>Endereço</dt><dd>${escapar(enderecoCompleto(u))}</dd></div><div><dt>Razão social</dt><dd>${escapar(u.razaoSocial || 'Não informada')}</dd></div><div><dt>Natureza</dt><dd>${escapar(u.classificacaoNatureza)} · código ${escapar(u.naturezaJuridica || 'não informado')}</dd></div><div><dt>Gestão / esfera</dt><dd>${escapar(u.gestao || 'Não informada')} / ${escapar(u.esfera || 'Não informada')}</dd></div><div><dt>Turno</dt><dd>${escapar(u.turnoAtendimento || 'Não informado')}</dd></div><div><dt>Atualização cadastral</dt><dd>${escapar(formatarData(u.atualizadoEm))}</dd></div></dl>
